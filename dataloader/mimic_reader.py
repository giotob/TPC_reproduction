...
# Minor changes were applied but most of the source code comes from the repository.
# https://github.com/EmmaRocheteau/TPC-LoS-prediction/blob/master/MIMIC_preprocessing/reader.py
# Under MIT License
...
def get_cols():
    with open('specified_cols.pkl','rb') as f:
...
class MIMICReader:
    ...
    def __init__(self, data_path, device=None):
        ...
        self.flat = self.flat.drop(['gender','age','first_careunit',	'admission_location', 'insurance','height' ,'weight'],axis=1) # columns had to be dropped, didnt preprocessed right
        # we minus 2 to calculate F because hour and time are not features for convolution
        self.F = (pd.read_csv(self._timeseries_path,index_col='patient', nrows=1).shape[1] - 1)//2
    ...
    def pad_sequences(self, ts_batch):
        ...
        padded = [patient + [[0] * (self.F * 2 + 1)] * (max_len - len(patient)) for patient in ts_batch]
        padded = torch.tensor(padded, device=self._device).type(self._dtype).permute(0, 2, 1)
